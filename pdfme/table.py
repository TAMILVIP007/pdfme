...
class PDFTable:
    ...
    * ``'::2;:'`` to modify all of the fills horizontally, every two rows. This
    ...
    def run(
        ...
        self.rowspan = {}
        self.vert_lines = [
            {'list': [], 'interrupted': True} for _ in range(self.cols_count + 1)
        ]
        ...
        for col in range(self.cols_count):
            self.x_abs = self.x + sum(self.widths[:col]) * self.width
    ...
    def add_row(self, row: Iterable, is_delayed: bool=False) -> str:
        ...
        for col, element in enumerate(row):
            if move_next := self.add_cell(col, element, is_delayed):
                continue
    ...
    def get_cell_style(self, element: CellType) -> tuple:
        ...
        if isinstance(element, dict) and 'delayed' in element:
            ...
            if keys := [key for key in element.keys() if key.startswith('.')]:
    ...
    def add_cell(self, col: int, element: CellType, is_delayed: bool) -> bool:
        ...
        real_height = 0
        did_finished = False
        if keys or self._is_delayed_type(element, 'text'):
    ...
    def process_image(
        ...
        """
        pdf_image = element.get('delayed', PDFImage(element['image']))
        img_width = width
